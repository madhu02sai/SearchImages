...
namespace FlickrWebApplicationRazorPages.Tests.Flickr
{
    class FlickrServiceTests
    {
        ...
        public async Task GetImagesByTag_WhenCalled_GivesCorrectResponseAsync()
        {
            ...
            Assert.AreEqual(10, Photos.Photos.Photo.Count());
            Assert.AreEqual("51352778937", Photos.Photos.Photo[0].Id);
            Assert.AreEqual("1964531968", Photos.Photos.Photo[0].Secret);
            Assert.AreEqual("65535", Photos.Photos.Photo[0].Server);
        }
    }
}
